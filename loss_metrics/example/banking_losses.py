...
def main():
    RAW_DATASET_PATH = Path("../../anjana_anonymizer/banking-simplified.csv")
    ...
    adult_dghs = get_csv_banking_dghs()
    k_values = [2**i for i in range(0, 11)]  # k = 1, 2, 4, ..., 1024
    results = []

    for k in k_values:
        ANONYMIZED_DATASET_PATH = Path(
            f"../../anjana_anonymizer/banking_simplified_anonymized_clean/banking_k{k}.csv"
        )
    ...
    result_df.to_csv("banking_loss_metrics_report.csv", index=False)
